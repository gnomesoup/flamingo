from pyrevit import DB, HOST_APP, revit, forms
from math import ceil
from string import ascii_uppercase

doc = HOST_APP.doc

scriptTitle = "Renumber Views on Sheet"
currentView = doc.ActiveView
titleBlocks = DB.FilteredElementCollector(doc, currentView.Id)\
                .OfCategory(DB.BuiltInCategory.OST_TitleBlocks)\
                .ToElements()

def GetSheetWorkArea(
    titleBlock,
    marginleft,
    marginright,
    margintop,
    ...
    width = titleBlock.get_Parameter(
        DB.BuiltInParameter.SHEET_WIDTH
    ).AsDouble()
    height = titleBlock.get_Parameter(
        DB.BuiltInParameter.SHEET_HEIGHT
    ).AsDouble()
    ...
    return {
        "MinimumPoint":(workMinX, workMinY), "MaximumPoint":(workMaxX, workMaxY)
    }

def inBoundingBox(boundingBox, point):
    return boundingBox[0][0] < point.X < boundingBox[1][0] and \
        boundingBox[0][1] < point.Y < boundingBox[1][1]

if len(titleBlocks) > 1:
    forms.alert("Found more than one title block on the sheet.\n\n"
           + "Remove extra titleblocks and try again",
           title=scriptTitle, exitscript=True)
...
workArea = GetSheetWorkArea(titleBlock,
    marginLeft / 12.0,
    marginRight / 12.0,
    marginTop /12.0,
    marginBottom / 12.0)
...
for viewportId in viewportIds:
    ...
    if viewport.Name != "0 No Title":
        ...
        labelMinPoint = labelOutline.MinimumPoint\
            + DB.XYZ(0.25/12.0, 0.25/12.0, 0)
        viewports[viewportId] = {"point": labelMinPoint,
                                 "parameter": parameter,
...
if not viewports:
    if not viewportIds:
        ...
        message = "There are no views on this sheet with a title shown.\n\n"\
